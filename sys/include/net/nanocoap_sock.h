...
#  endif
        ...
        struct {
            ...
            mutex_t ws_sync;            /**< unlock this on when a reply is
                                             ready */
            int ws_retval;              /**< return value to pass along */
        };
#endif
    };
...
/**
 * @brief   Check if the given packet is a duplicate of the one the
 *          separate response content contains was prepared for
 *
 ...
 */
int nanocoap_server_send_separate(const nanocoap_server_response_ctx_t *ctx, unsigned code,
                                  unsigned type, const void *payload, size_t len);
/**
 ...
 */
static inline int nanocoap_server(sock_udp_ep_t *local, uint8_t *buf, size_t bufsize)
{
 ...
 *
 * @param[in]   sock        Socket the packet is inteded for
