...
#include "random.h"
#include "sys/uio.h"  /* IWYU pragma: keep (false positive) */
#include "ztimer.h"
                               ...
                               const sock_udp_ep_t *remote, credman_tag_t tag)
{
    ...
    (void)remote;
    (void)tag;
    extern void nanocoap_sock_dtls_connect_called_but_module_nanocoap_dtls_not_used(void);
    nanocoap_sock_dtls_connect_called_but_module_nanocoap_dtls_not_used();
    return -ENOTSUP;
#endif
}
...
static void _client_ws_conn_recv_cb(coap_ws_conn_t *conn, void *msg, size_t msg_len)
{
    ...
    DEBUG_PUTS("nanocoap_ws: Unexpected response");
}
...
struct sock_and_ztimer {
    nanocoap_sock_t *sock;
    ztimer_t timer;
};

static void _sock_recv_timeout_cb(void *_arg)
{
    struct sock_and_ztimer *arg = _arg;
    mutex_unlock(&arg->sock->ws_sync);
}
                                           ...
                                           coap_request_cb_t cb, void *arg)
{
    ...
    int err = sock->ws_conn->handle->transport->sendv(sock->ws_conn, &iol);
    if (err) {
        irq_state = irq_disable();
        sock->ws_cb = NULL;
        sock->ws_cb_arg = NULL;
        irq_restore(irq_state);
        DEBUG("nanocoap_ws: sending request failed: %d\n", err);
        return err;
    }

    struct sock_and_ztimer saz = {
        .sock = sock,
        .timer = {
            .callback = _sock_recv_timeout_cb,
            .arg = &saz,
        },
    };
    if (IS_USED(MODULE_ZTIMER_MSEC)) {
        ztimer_set(ZTIMER_MSEC, &saz.timer, 5 * MS_PER_SEC);
    }
    else {
        ztimer_set(ZTIMER_USEC, &saz.timer, 5 * US_PER_SEC);
    }

    mutex_lock(&sock->ws_sync);
    ztimer_remove(IS_USED(MODULE_ZTIMER_MSEC) ? ZTIMER_MSEC : ZTIMER_USEC, &saz.timer);
    err = sock->ws_retval;
    DEBUG("nanocoap_sock_ws_request_cb returns %d for conn %p\n",
                                ...
                                coap_request_cb_t callback, void *arg)
{
    ...
    ssize_t hdr_size = nanocoap_sock_build_pkt(req->sock, NULL, buf, sizeof(buf),
                                               COAP_TYPE_CON, NULL, 0,
                                               req->method);
    assume(hdr_size > 0);
    pktpos += hdr_size;
...
int nanocoap_sock_url_connect(const char *url, nanocoap_sock_t *sock)
{
    ...
    if (type == COAP_SOCKET_TYPE_TCP) {
        ...
        if (!remote.udp.port) {
            remote.udp.port = (type == COAP_SOCKET_TYPE_DTLS) ? COAPS_PORT
                                                              : COAP_PORT;
        }
    }
                                  ...
                                  sock_async_flags_t flags, void *arg)
{
    ...
    ssize_t len;
    if ((len = nanocoap_send_csm_message(client, ctx->tx_buf, sizeof(ctx->tx_buf))) < 0) {
...
#if MODULE_NANOCOAP_UDP
int nanocoap_server_udp(sock_udp_ep_t *local, uint8_t *buf, size_t bufsize)
{
    sock_udp_t sock;
                                     ...
                                     coap_pkt_t *pkt, const coap_request_ctx_t *req)
{
    ...
    coap_transport_t transport = coap_get_transport(pkt);
    ...
    case COAP_TRANSPORT_UDP:
        ...
        assert(req->remote_udp && req->local_udp);
                                           ...
                                           coap_pkt_t *pkt, const coap_request_ctx_t *req)
{
    ...
    /* req is only accessed when UDP support is compiled in */
    (void)req;
...
static int _nanocoap_server_send_separate_udp(const nanocoap_server_response_ctx_t *ctx,
                                              unsigned code, unsigned type,
                                              const void *payload, size_t len)
{
    if (!IS_USED(MODULE_NANOCOAP_UDP)) {
        return -ENOTSUP;
    }
    ...
    ssize_t rbuf_len = coap_build_udp_hdr(rbuf, sizeof(rbuf), type,
                                          ctx->token, ctx->tkl,
                                          code, random_uint32());
    ...
    const sock_udp_ep_t *local = NULL, *remote = NULL;
#if MODULE_NANOCOAP_UDP
    local = &ctx->local_udp;
    remote = &ctx->remote_udp;
#endif

    /* this should not be reached unless MODULE_NANOCOAP_UDP is 1 */
    assert(local && remote);
    ...
    if (!sock_udp_ep_is_multicast(local)) {
        aux_out_ptr = &aux_out;
    }
    return sock_udp_sendv_aux(NULL, &head, remote, aux_out_ptr);
}

static int _nanocoap_server_send_separate_tcp(const nanocoap_server_response_ctx_t *ctx,
                                              unsigned code, const void *payload, size_t len)
{
    sock_tcp_t *sock = NULL;
#if MODULE_NANOCOAP_TCP
    sock = ctx->sock_tcp;
#endif
    if (!IS_USED(MODULE_NANOCOAP_TCP)) {
        return -ENOTSUP;
    }
    uint8_t rbuf[COAP_TCP_TENTATIVE_HEADER_SIZE + sizeof(ctx->token) + 1];
...
static int _nanocoap_server_send_separate_ws(const nanocoap_server_response_ctx_t *ctx,
                                             unsigned code, const void *payload, size_t len)
{
    coap_ws_conn_t *conn = NULL;
#if MODULE_NANOCOAP_WS
    conn = ctx->conn_ws;
#endif
    if (!IS_USED(MODULE_NANOCOAP_WS)) {
        return -ENOTSUP;
    }
    uint8_t rbuf[COAP_WS_HEADER_SIZE + sizeof(ctx->token) + 1];

    ssize_t rbuf_len = coap_build_ws_hdr(rbuf, sizeof(rbuf), ctx->token, ctx->tkl, code);
    ...
    return conn->handle->transport->sendv(conn, &head);
}
                                  ...
                                  const void *payload, size_t len)
{
    switch (nanocoap_server_response_ctx_transport(ctx)) {
    case COAP_TRANSPORT_DTLS:
        assert(0); /* TODO: Impelement me */
        return -ENOTSUP;
    default:
    case COAP_TRANSPORT_UDP:
        return _nanocoap_server_send_separate_udp(ctx, code, type, payload, len);
    case COAP_TRANSPORT_TCP:
        return _nanocoap_server_send_separate_tcp(ctx, code, payload, len);
    case COAP_TRANSPORT_WS:
        return _nanocoap_server_send_separate_ws(ctx, code, payload, len);
    }
    (void)type;
}
#endif
