...
ifneq (, $(_BOARD_ALIAS_USED))
  ...
  _whitelist := $(BOARDS_SUPPORTED)
  _blacklist := $(BOARDS_UNSUPPORTED)
  _test_blacklist := $(TEST_ON_CI_BLACKLIST)
  BOARDS_SUPPORTED += $(if $(filter $(_alias),$(_whitelist)), $(_board))
  BOARDS_UNSUPPORTED += $(if $(filter $(_alias),$(_blacklist)), $(_board))
