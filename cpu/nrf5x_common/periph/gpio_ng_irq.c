 ...
 * @ingroup     drivers_periph_gpio_ng_irq
 * @{
 *
 * @file
...
/**
 * @brief   select a GPIOTE channel suitable for managing the irq for the given pin
 *
 * @return  if one channel is already used for the given pin, return that. otherwise return
 *          a free channel
 * @retval  GPIOTE_CHAN_NUMOF   all GPIOTE channels occupied by pins different to the selected one
 */
